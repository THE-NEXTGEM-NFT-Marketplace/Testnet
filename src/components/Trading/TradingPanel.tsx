...
export function TradingPanel({ market }: TradingPanelProps) {
  ...
  const { buyShares, sellShares: sellSharesAction, positions, usdcBalance, lastClaimTimestamp, setCurrentView } = useAppStore();
  const now = Date.now();
  ...
  const handleBuy = async () => {
    ...
    setIsLoading(true);
    const loadingToast = toast.loading("Executing buy transaction...");

    try {
      // Execute gas-only transaction; no real payment
      await createGasOnlyTx(signAndExecuteTransactionBlock);
      
      // Update local state
      buyShares(market.id, selectedOutcome, amount);
      setBuyAmount('');
      
      toast.dismiss(loadingToast);
      toast.success(`Bought ${sharesReceived.toFixed(2)} ${selectedOutcome} shares for $${amount.toFixed(2)}`);
    } catch (error) {
  ...
  const handleSell = async () => {
    ...
    setIsLoading(true);
    const loadingToast = toast.loading("Executing sell transaction...");
    ...
    try {
      ...
      // Update local state
      sellSharesAction(market.id, selectedOutcome, shares);
      setSellShares('');
      
      const saleAmount = shares * currentPrice;
      toast.dismiss(loadingToast);
      toast.success(`Sold ${shares.toFixed(2)} ${selectedOutcome} shares for $${saleAmount.toFixed(2)}`);
    } catch (error) {
