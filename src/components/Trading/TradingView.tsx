...
export function TradingView() {
  const { currentMarket, setCurrentMarket, setCurrentView } = useAppStore();
  ...
  // Dynamic price illusion
  const { yesPrice, noPrice } = useDynamicPrice(currentMarket.yesPrice);

  // Push dynamic prices into the global store to simulate live PnL
  React.useEffect(() => {
    ...
    useAppStore.setState((state) => {
      const updatedMarkets = state.markets.map((m) => {
        ...
        const newHistory = [...m.priceHistory, { time: Date.now(), value: Number(yesPrice.toFixed(4)) }];
        // keep last 120 points (~5 minutes at 2.5s interval)
      ...
      const updatedCurrent = updatedMarkets.find((m) => m.id === marketId) || state.currentMarket;
      return { markets: updatedMarkets, currentMarket: updatedCurrent };
    });
  }, [yesPrice, noPrice, currentMarket?.id]);
